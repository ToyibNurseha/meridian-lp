...
export function getLessonsForPrompt(opts = {}) {
  ...
  const roleMatched = data.lessons
    .filter((l) => {
      ...
      const tagsArr = Array.isArray(l.tags) ? l.tags : (l.tags ? [l.tags] : []);
      const tagOk  = roleTags.length === 0 || !tagsArr.length || tagsArr.some((t) => roleTags.includes(t));
      return roleOk && tagOk;
    })
    .sort(byPriority)
    .slice(0, ROLE_CAP);
