...
const SCREENER_TOOLS = new Set(["deploy_position", "get_active_bin", "get_top_candidates", "check_smart_wallets_on_pool", "get_token_holders", "get_token_narrative", "get_token_info", "search_pools", "get_pool_memory", "get_wallet_balance", "get_my_positions", "study_top_lpers"]);
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  const { interactive = false, onToolStart = null, onToolFinish = null, noForceFirstTool = false } = options;
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      const FALLBACK_MODEL = "mimo-v2-flash";
      let response;
      let usedModel = activeModel;
      // Force a tool call on step 0 for action intents — prevents the model from inventing deploy/close outcomes
      const ACTION_INTENTS = /\b(deploy|open|add liquidity|close|exit|withdraw|claim|swap|block|unblock)\b/i;
      let toolChoice = (step === 0 && !noForceFirstTool && (ACTION_INTENTS.test(goal) || mustUseRealTool)) ? "required" : "auto";
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        try {
          ...
          const thrownCode = error?.status || error?.code;
          if (thrownCode === 502 || thrownCode === 503 || thrownCode === 529) {
            const wait = (attempt + 1) * 5000;
            if (attempt === 1 && usedModel !== FALLBACK_MODEL) {
              usedModel = FALLBACK_MODEL;
              log("agent", `Switching to fallback model ${FALLBACK_MODEL} after thrown ${thrownCode}`);
            } else {
              log("agent", `Provider threw ${thrownCode}, retrying in ${wait / 1000}s (attempt ${attempt + 1}/3)`);
      ...
      const toolResults = await Promise.all(msg.tool_calls.map(async (toolCall) => {
        ...
        // Lock deploy_position after first attempt unless executor blocked it pre-flight.
        // blocked:true = no on-chain action attempted = safe to retry with corrected args.
